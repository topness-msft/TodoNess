...
## Step 1: Load waiting tasks

Use the Bash tool to run this Python script to get all waiting tasks:
...
rows = conn.execute(\"SELECT id, title, description, key_people, source_type, source_id, created_at FROM tasks WHERE status = 'waiting'\").fetchall()
for r in rows:
    print(json.dumps({'id': r['id'], 'title': r['title'], 'key_people': r['key_people'] or '', 'source_type': r['source_type'] or 'manual', 'source_id': r['source_id'] or '', 'created_at': r['created_at']}))
conn.close()
"
```

If there are zero waiting tasks, print "No waiting tasks to check." and stop.
...
Then build the WorkIQ query using the **source channel** to focus the search:
...
Review the WorkIQ results against the task's title and description. Classify:
...
for task_id, status, summary in results:
    val = json.dumps({'status': status, 'summary': summary, 'checked_at': now})
...
Replace TASK_ID, STATUS, SUMMARY with actual values for each task.
